#
#             Zhadevv Project
#             --MIT License--
#
# Feed Me Starnya Bang:>
# Project 100% Open Source
# Bebas Recode, Deploy Production. KECUALI
# Diperjual-Belikan.
#
# Project ini Sepenuhnya Gratis, Makannua ksih Bintang Dong anj:>
# *bercanda ajahh
#
# Regards
# Zhadevv
#

from fastapi import APIRouter, Query, HTTPException
from app.api.models.BaseResponse import BaseResponse, ErrorResponse
from app.api.models.DonghuaModel import DonghuaDetailResponse
from app.api.models.parser.DonghuaParser import DonghuaParser

router = APIRouter()
parser = DonghuaParser()

@router.get(
    "/detail/{slug}",
    response_model=BaseResponse[DonghuaDetailResponse],
    summary="Donghua Detail",
    description="Get detailed information about a specific donghua"
)
async def get_donghua_detail(
    slug: str,
    apikey: str = Query(None, description="Optional")
):
    try:
        if not slug or slug.strip() == "":
            return ErrorResponse(
                status=400,
                success=False,
                message="Slug cannot be empty"
            )
            
        result = parser.parse_detail(slug.strip())
        
        if "error" in result and result["error"]:
            return ErrorResponse(
                status=404,
                success=False,
                message=result["error"]
            )
            
        response_data = DonghuaDetailResponse(**result)
        
        return BaseResponse[DonghuaDetailResponse](
            status=200,
            success=True,
            data=response_data
        )
        
    except Exception as e:
        return ErrorResponse(
            status=500,
            success=False,
            message=f"Internal server error: {str(e)}"
        )