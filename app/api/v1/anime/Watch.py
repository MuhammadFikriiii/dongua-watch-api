...
from app.api.models.AnimeModel import AnimeWatchResponse
...
@router.get(
    "/watch/{slug}/{episode}",
    response_model=BaseResponse[AnimeWatchResponse],
    summary="Watch Anime Episode",
    description="Get streaming links and download options for an anime episode"
)
async def watch_anime_episode(
    slug: str,
    episode: str,
    ...
    try:
        ...
        response_data = AnimeWatchResponse(**result)
        
        return BaseResponse[AnimeWatchResponse](
            status=200,
            success=True,
            data=response_data
        )
