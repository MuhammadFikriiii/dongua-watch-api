...
@router.get(
    "/filters/value",
    response_model=BaseResponse[AnimeFiltersResponse],
    summary="Advanced Search Filters",
    description="Get available filters for advanced anime search"
)
async def get_anime_filters(
    ...
    try:
        ...
        response_data = AnimeFiltersResponse(**result)
        
        return BaseResponse[AnimeFiltersResponse](
            status=200,
            success=True,
            data=response_data
        )
...
@router.get(
    ...
    description="Get anime list in text mode for advanced search"
)
async def get_anime_filters_text_mode(
    apikey: str = Query(None, description="Optional")
):
    try:
...
@router.get(
    ...
    description="Search anime using advanced filters"
)
@router.get(
    "/filters/{page}",
    response_model=BaseResponse[AnimeListResponse],
    summary="Advanced Search with Page",
    description="Search anime using advanced filters with pagination"
)
async def advanced_search_anime(
    status: Optional[str] = Query(None, description="Anime status filter"),
